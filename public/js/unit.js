...
$('#save,#update').click(function (e) {
    ...
    if(this_id=="save")  //Save start
    {
     if(confirm("Do You Wants to Save Record ?")){
        e.preventDefault();
        data = new FormData($('#unit-form')[0]);//form name
        ...
        success: function(result){
            ...
            if(result=="success")
            {
                window.location=base_url+"unit/add";
                return;
            }
...
function delete_unit(unit_id)
{
   ...
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post("unit/removeUnit",{'unit_id': unit_id, 'csrf_token': csrf_token},function(result){

    if(result=="success")
        {
            toastr["success"]("Record Deleted Successfully!");
            $('#unit_list').DataTable().ajax.reload();
        }
        else if(result=="failed"){
