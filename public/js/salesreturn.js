...
function delete_payment(payment_id){
 if(confirm("Do You Wants to Delete Record ?")){
    var base_url=$("#baseURL").val().trim();
    $(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post(base_url+"salesReturn/deletePayment",{payment_id:payment_id},function(result){
   //alert(result);return;
   result=result.trim();
     if(result=="success")
        { 
...
function pay_now(return_id){
  $.post('salesReturn/showPayNowModal', {return_id: return_id}, function(result) {
    $(".view_payments_modal").html('').html(result);
    showModal();
  });
}
function view_payments(sales_id){
  $.post('salesReturn/viewPaymentsModal', {sales_id: sales_id}, function(result) {
    $(".view_payments_modal").html('').html(result);
    showModal();
  });
}
...
function save_payment(return_id){
 ...
 let amt = $("#due_amount_temp").html().trim();
    ...
    $.post('salesReturn/savePayment', {return_id: return_id,payment_type:payment_type,amount:amount,payment_date:payment_date,payment_note:payment_note}, function(result) {
      result=result.trim();
  //alert(result);return;
        if(result=="success")
        {
